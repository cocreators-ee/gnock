// https://github.com/brunnerh/screen-recorder/blob/f06e3490e3b5f04a0eaa451328b6cbb885a1c291/src/plugins/extract-fonts.ts
...
export function extractFonts(options: ExtractFontsOptions): Plugin {
  const { cacheDir, command } = options
  const fonts = new Set<string>()

  return {
    name: "fonts-extractor",
    async transform(code, id) {
      if (extname(id) != ".css") return

      if (shell.test("-d", cacheDir) == false) shell.mkdir(cacheDir)
      ...
      const replacedSource = code.replaceAll(search, (match, url) => {
        ...
        return command == "serve" ? `url(/${cacheDir}/${name})` : `url(./${name})`
      })
